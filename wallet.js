 ...
 * 
 * Manages both in-browser "Quick Wallet" and external wallets via Web3Modal.
...
const WALLETCONNECT_PROJECT_ID = 'aee6080680dc3a81dfecc8f941533fc3';
...
async function _updateWalletUI() {
    ...
    const useInBrowserWalletBtn = document.getElementById('use-in-browser-wallet-btn');
    ...
    if (activeWalletType) {
        ...
        if (activeWalletType === 'in-browser') {
            currentAddress = inBrowserWalletDetails.address;
        } else {
            try {
        ...
        useInBrowserWalletBtn.style.display = 'inline-block';
...
// Function to show the Quick Wallet explanation modal
function showQuickWalletExplanation() {
    // Make sure we have a valid address before showing the modal
    if (!inBrowserWalletDetails || !inBrowserWalletDetails.address) {
        console.error("Cannot show Quick Wallet explanation: wallet address is not available");
        return;
    }
    
    // Update the address in the modal
    const addressEl = document.getElementById('modal-wallet-address');
    if (addressEl) {
        addressEl.textContent = inBrowserWalletDetails.address;
    }
    
    // Initialize and show the modal
    const modalElement = document.getElementById('quick-wallet-explanation');
    if (!modalElement) {
        console.error("Quick wallet explanation modal not found in DOM");
        return;
    }
    ...
    const copyBtn = document.getElementById('modal-copy-address');
    const saveKeyBtn = document.getElementById('modal-save-key');
    ...
    if (copyBtn) {
        ...
        newCopyBtn.addEventListener('click', function() {
            const walletAddress = inBrowserWalletDetails.address;
            ...
            navigator.clipboard.writeText(walletAddress)
                ...
                .catch(err => {
                    ...
                    alert('Error copying address: ' + err.message);
                });
        });
    }
    ...
    if (saveKeyBtn) {
        // Clone and replace to remove existing event listeners
        const newSaveKeyBtn = saveKeyBtn.cloneNode(true);
        saveKeyBtn.parentNode.replaceChild(newSaveKeyBtn, saveKeyBtn);
        
        // Add fresh event listener
        newSaveKeyBtn.addEventListener('click', function() {
            if (confirm('This will copy your PRIVATE KEY to clipboard. Never share your private key with anyone! Are you sure you want to proceed?')) {
                const privateKey = inBrowserWalletDetails.privateKey;
...
// Update the activateInBrowserWallet function to call showQuickWalletExplanation for new wallets
async function activateInBrowserWallet(isAutoConnectAttempt = false) {
    if (_initializeInBrowserWallet()) {
        activeWalletType = 'in-browser';
        window.activeWalletType = activeWalletType; // Export to global for trollbox
        localStorage.setItem(LOCALSTORAGE_KEYS.PREFERRED_WALLET_TYPE, 'in-browser');
        ...
        // Ensure ethereumClientInstance is properly set up for in-browser wallet
        ethereumClientInstance = {
            getAccount: () => ({
                isConnected: inBrowserWalletDetails.isInitialized,
                address: inBrowserWalletDetails.address,
            ...
            // Add methods that might be called from UI code
            getSigner: () => ethersSigner,
            switchChain: async () => ({ id: BASE_CHAIN_ID }),
            disconnect: async () => {
                // Will be implemented in disconnectInBrowserWallet
        ...
        // Check if this is a new wallet (first time activation) and not an auto-connect
        const isNewWallet = !localStorage.getItem(LOCALSTORAGE_KEYS.IN_BROWSER_WALLET + '_initialized');
        if (isNewWallet && !isAutoConnectAttempt) {
            // Mark as initialized to prevent showing the explanation again
            localStorage.setItem(LOCALSTORAGE_KEYS.IN_BROWSER_WALLET + '_initialized', 'true');
            
            // Show explanation after everything is initialized and UI is updated
            // Increased timeout to ensure wallet is fully initialized
            setTimeout(() => {
                showQuickWalletExplanation();
            }, 1000);
        }
        ...
        return true;
    } else if (!isAutoConnectAttempt) {
        alert("Could not initialize in-browser wallet.");
        await _updateWalletUI(); // Show connect options
        return false;
    }
    return false;
}

async function disconnectInBrowserWallet() {
    // Optional: Ask user if they want to delete the stored key, or just disconnect
    // For now, we just disconnect, key remains for next session unless explicitly deleted by user action (not implemented here)
    // To fully "forget" and force new key generation:
    // localStorage.removeItem(LOCALSTORAGE_KEYS.IN_BROWSER_WALLET);
    // inBrowserWalletDetails = { privateKey: null, address: null, isInitialized: false };
...
async function initializeWalletSystem() {
    ...
    $('#use-in-browser-wallet-btn').on('click', () => activateInBrowserWallet(false));
    $('#disconnect-wallet-btn').on('click', async () => {
        if (activeWalletType === 'in-browser') {
            await disconnectInBrowserWallet();
        } else if (activeWalletType === 'external') {
            await disconnectExternalWallet();
        }
    });
    ...
    $('#use-in-browser-wallet-btn').show();

    const preferredType = localStorage.getItem(LOCALSTORAGE_KEYS.PREFERRED_WALLET_TYPE);
    
    if (preferredType === 'in-browser') {
        await activateInBrowserWallet(true); // true for auto-connect attempt
    } else if (preferredType === 'external') {
