#!/usr/bin/env python3
"""
Odette Analytics Prototype v2
Implements proper indicator calculations based on specifications:

- 1D Max/Min: Yesterday's session high/low or rolling 24h high/low
- Call/Put Resistance/Support: Largest outstanding OI, delta-adjusted
- Gamma Wall 0DTE: Strike with largest net dealer gamma for today's expiries  
- HVL: High-Volume Level from clustered futures volume data
"""
...
class DeribitAnalyticsV2:
    """Enhanced analytics engine for Deribit options data"""
    ...
    async def fetch_instruments_with_oi(self, currency: str = "BTC") -> List[Dict]:
        ...
        # Fetch open interest for each instrument
    ...
    async def generate_key_levels(self, currency: str = "BTC") -> List[KeyLevel]:
        ...
        try:
            ...
            spot_price, stats_24h, instruments, futures_trades = await asyncio.gather(
            ...
            for i, result in enumerate([spot_price, stats_24h, instruments, futures_trades]):
                if isinstance(result, Exception):
                    print(f"Error in data fetch {i}: {result}")
                    return []
...
async def main():
    ...
    async with DeribitAnalyticsV2() as analytics:
        for currency in currencies:
            print(f"\n{'='*60}")
            print(f"Enhanced Key Levels for {currency}")
            print(f"{'='*60}")
            ...
            try:
                ...
                with open(f"odette/{currency.lower()}_enhanced_levels.json", "w") as f:
                    json.dump(json_data, f, indent=2)
                
                print(f"\n💾 Enhanced data exported to {currency.lower()}_enhanced_levels.json")
