/**
 * Deribit Analytics - Enhanced Node.js Version
...
class DeribitAnalytics {
    ...
    /**
     * HTTP request using Node.js built-in https module
     */
    httpRequest(url, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            const requestOptions = {
                ...
                headers: {
                    'User-Agent': 'Node.js Analytics Client',
            ...
            const req = https.request(requestOptions, (res) => {
                ...
                if (res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307 || res.statusCode === 308) {
                    ...
                    if (redirectUrl) {
                        ...
                        return this.httpRequest(redirectUrl, options).then(resolve).catch(reject);
                    }
                }
    ...
    /**
     * Format results for display (matching Python output format)
     */
    formatResults(result) {
        console.log(`\n=== ${result.currency} Results ===`);
        console.log(`Spot Price: $${result.spot_price.toLocaleString()}`);

        for (const [levelName, levelData] of Object.entries(result.levels)) {
            const price = levelData.price;
            const pct = levelData.percentage;
            console.log(`${levelName}: $${price.toLocaleString()} (${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%)`);
        }
    }
}
